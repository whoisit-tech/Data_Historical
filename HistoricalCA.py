...
def preprocess_data(df):
    ...
    # SLA history Pending CA -> Pending CA Completed
    if all(c in df.columns for c in ['apps_id', 'apps_status', 'action_on_parsed']):
        sla_hist_list = []
        df_sorted = df.sort_values('action_on_parsed')

        for app_id, sub in df_sorted.groupby('apps_id'):
            sub = sub.dropna(subset=['apps_status', 'action_on_parsed'])

            pending_rows = sub[sub['apps_status'].str.upper() == 'PENDING CA']
            if pending_rows.empty:
                continue
            start_row = pending_rows.iloc[0]

            completed_rows = sub[
                (sub['action_on_parsed'] > start_row['action_on_parsed']) &
                (sub['apps_status'].str.upper() == 'PENDING CA COMPLETED')
            ]
            if completed_rows.empty:
                continue

            end_row = completed_rows.iloc[0]

            sla_days = calculate_working_days(
                start_row['action_on_parsed'],
                end_row['action_on_parsed']
            )

            sla_hist_list.append({
                'apps_id': app_id,
                'SLA_Pending_CA_Days': sla_days
            })

        if sla_hist_list:
            sla_hist_df = pd.DataFrame(sla_hist_list).drop_duplicates('apps_id')
            df = df.merge(sla_hist_df, on='apps_id', how='left')
    ...
    with tab1:
        ...
        if 'OSPH_Category' in df_filtered.columns and 'Scoring_Detail' in df_filtered.columns:
            ...
            for osph in sorted([x for x in df_filtered['OSPH_Category'].unique() if x != 'Unknown']):
                ...
                for scoring in ['-', 'APPROVE', 'Approve 1', 'Approve 2', 'REGULER', 'Reguler',
                                'Reguler 1', 'Reguler 2', 'REJECT', 'Reject', 'Reject 1',
                                'Reject 2', 'Scoring in Progress', 'data historical']:
    ...
    with tab2:
        ...
        with col1:
            ...
            if 'LastOD_clean' in df_filtered.columns:
                ...
                if len(lastod_df) > 0:
                    ...
                    st.markdown(
                        f'<div class="warning-card">**Critical**: {worst["LastOD Range"]} has {worst["Reject%"]} rejection rate with avg risk score {worst["Avg Risk"]}</div>',
                        unsafe_allow_html=True
                    )
        ...
        if 'LastOD_clean' in df_filtered.columns and 'max_OD_clean' in df_filtered.columns:
            ...
            if len(improving) > 0:
                ...
                st.markdown(
                    f'<div class="success-card">**Insight**: Customers with improving OD trend have {approve_rate:.1f}% approval rate - OD improvement is a strong positive indicator!</div>',
                    unsafe_allow_html=True
                )
